...
public class CameraMovement : MonoBehaviour
{
		...
		public Transform bomb;			// The target of the camera at runtime of final level.
		...
		void TrackBomb ()
		{
			...
			// ... the target x coordinate should be a Lerp between the camera's current x position and the player's current x position.
						targetX = Mathf.Lerp (transform.position.x, bomb.position.x, xSmooth * Time.deltaTime);
		
				// If the player has moved beyond the y margin...
				if (CheckYMargin ())
			// ... the target y coordinate should be a Lerp between the camera's current y position and the player's current y position.
						targetY = Mathf.Lerp (transform.position.y, bomb.position.y, ySmooth * Time.deltaTime);
